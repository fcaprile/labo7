# -*- coding: utf-8 -*-
"""
Created on Fri Oct 25 15:53:19 2019
...
def curva_por_carpeta(carpeta_base,tinf,tsup,fc=1,filtrar=False):
    indice_carpetas=[]
...
#                corr=R.y[pico_corriente]/altura_pico_bobina/5
...
#            t2=6*10**-6
    ...
    corriente_media=np.array(corriente_media)*np.mean(descarga_media)
    error_corriente_media=np.array(error_corriente_media)*np.mean(descarga_media)
#    plt.plot(descarga_media)
#    tensiones=[]
#    for i in indice_carpetas:
...
#%%
#analizo
#carpeta_base1='C:/Users/DG/Documents/GitHub/labo7/mediciones/iman/10-25/'
...
t1,c1,tm1,cm1,et1,ec1=curva_por_carpeta(carpeta_base1,tinf,tsup,fc,filtrar=True)#,sacar_outliers=True)
t2,c2,tm2,cm2,et2,ec2=curva_por_carpeta(carpeta_base2,tinf,tsup,fc,filtrar=False)#,sacar_outliers=True)
...
'''
np.savetxt('curva carac 1000V con t entre 1 y 3 config 2.txt',[tensiones,corrientes,error_corrientes], delimiter='\t')
