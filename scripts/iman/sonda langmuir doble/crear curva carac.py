...
#            t2=6*10**-6
        ...
        corriente_media.append(np.mean(corrientes_esta_carpeta)*np.mean(descarga_media))        
        tension_media.append(np.mean(tensiones_esta_carpeta))        
        error_corriente_media.append(np.std(corrientes_esta_carpeta)*np.mean(descarga_media)/np.sqrt(len(corrientes_esta_carpeta)))
...
t2,c2,tm2,cm2,et2,ec2=curva_por_carpeta(carpeta_base2,0.5*10**-6,1.5*10**-6,filtrar=False)#,sacar_outliers=True)
